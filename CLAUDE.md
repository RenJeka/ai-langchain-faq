...
docker compose up -d         # підняти Chroma (:8000) і docling-serve (:5001)
...
- **Chroma (Docker)**: `docker compose up -d` запускає два контейнери — `chromadb/chroma:0.5.23` (:8000) і `quay.io/docling-project/docling-serve:latest` (:5001). Дані зберігаються у `.chroma/` (git-ignored). Щоб перебудувати індекс — `npm run ingest`.
